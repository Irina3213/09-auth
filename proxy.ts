...
//       new URL(pathname + request.nextUrl.search, BACKEND_URL)
//     );
//   }
...
// // БЕЗ ЦЬОГО РЯДКА БУДЕ 404 НА VERCEL
// export default proxy;

// export const config = {
//   matcher: ["/auth/:path*", "/api/:path*", "/profile/:path*", "/notes/:path*"],
// };

import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";
import { checkSession } from "./lib/api/serverApi"; // Тепер ми її використаємо
...
export async function proxy(request: NextRequest) {
  ...
  // 1. Проксіювання запитів на бекенд (Виправляє 404)
  if (pathname.startsWith("/auth") || pathname.startsWith("/api")) {
    const targetUrl = new URL(pathname + request.nextUrl.search, BACKEND_URL);
    return NextResponse.rewrite(targetUrl);
  }
  ...
  // 2. ВИКОРИСТАННЯ checkSession (Прибирає вашу помилку)
  if (!accessToken && refreshToken) {
    try {
      // Викликаємо функцію, яку ви імпортували
      const res = await checkSession();
      sessionResponse = res as unknown as Response;
  ...
  // 4. Копіювання Set-Cookie заголовків (Вимога ментора)
  if (sessionResponse) {
    const setCookie = sessionResponse.headers.get("set-cookie");
    if (setCookie) {
      response.headers.set("set-cookie", setCookie);
